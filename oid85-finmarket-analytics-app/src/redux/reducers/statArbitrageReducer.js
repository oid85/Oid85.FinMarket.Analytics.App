import {
    ...
    FETCH_PORTFOLIO_REGRESSION_TAIL,
...
const initialState = {
    portfolioListData: {},
    portfolioRegressionTailData: {},
    portfolioMonitorData: {},
    portfolioBacktestData: {},
    portfolioOptimizationData: {},
    currentPortfolio: { name: 'StatArbitrage', description: 'Портфель StatArbitrage' }    
}

export const statArbitrageReducer = (state = initialState, action) => {
    switch (action.type) {

        case FETCH_PORTFOLIO_LIST:
            return {...state, portfolioListData: {...action.payload}}

        case FETCH_PORTFOLIO_REGRESSION_TAIL:
            return {...state, portfolioRegressionTailData: {...action.payload}}
