...
export const PortfolioPositionList = () => {
    ...
    const GetDeltaPercentValue = (deltaPercent) => {
        if (deltaPercent > 0.0) { return `(+) больше расч. на ${formatNumber(Math.abs(deltaPercent))} %` }
        if (deltaPercent < 0.0) { return `(-) меньше расч. на ${formatNumber(Math.abs(deltaPercent))} %` }
